...
public class VulkanFence {
	...
	public VulkanFence(VulkanLogicalDevice logicalDevice) {
		...
		int err = VK10.vkCreateFence(logicalDevice.getInternal(), createInfo, null, pHandle);
	...
	public void reset() {
		int err = VK10.vkResetFences(logicalDevice.getInternal(), handle);
		if(err != VK10.VK_SUCCESS) {
	...
	public void waitForFence(){		
		int err = VK10.vkWaitForFences(logicalDevice.getInternal(), pHandle, true, 1000000000l);
		if(err != VK10.VK_SUCCESS) {
			throw new AssertionError("Failed to reset Fence: "+VKUtil.translateVulkanResult(err));
		}
	}
	...
	public void destroy(){		
		VK10.vkDestroyFence(logicalDevice.getInternal(), handle, null);		
	}
