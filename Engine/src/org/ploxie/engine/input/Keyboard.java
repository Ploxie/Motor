...
public class Keyboard implements KeyListener {
	...
	private final List<KeyInput> pressedKeys = new ArrayList<KeyInput>(16);

	@Override
	public synchronized void onPress(int key, int mods, boolean repeat) {
		KeyInput keyInput = new KeyInput(key, mods);
		if (!pressedKeys.contains(keyInput)) {
			System.out.println("PRESS:" + keyInput);
			pressedKeys.add(keyInput);
		}
	}

	@Override
	public synchronized void onRelease(int key, int mods) {
		for (Iterator<KeyInput> iterator = pressedKeys.iterator(); iterator
				.hasNext();) {
			KeyInput keyInput = iterator.next();
	...
	public synchronized boolean isPressed(final int key) {
		return isPressed(key, -1);
	}

	public synchronized boolean isPressed(final int key,
			final boolean ignoreMods) {
		for (KeyInput k : pressedKeys) {
			if (k.getKey() == key) {
				return true;
			}
		}
	...
	public synchronized boolean isPressed(final int key, final int mods) {
		for (KeyInput k : pressedKeys) {
			if (k.getKey() == key) {
				if (mods == -1 || mods == k.getMods()) {
					return true;
				}
			}
		}
	...
	public static String getKeyText(int keyCode, int modifiers) {
		String m = getModifiersText(modifiers);
		String k = getKeyText(keyCode);
		if (m != null) {
			k = m + "+" + k;
		}
		return k;
